...
<form>
  ...
  class GraphView extends PhaseView {
      ...
      updateInputAndOutputBubbles() {
          ...
          s.classed("filledBubbleStyle", function (c) {
              ...
              if (components[0] == "ib") {
                  ...
                  return edge != undefined ? edge.isVisible() : true;
              }
              else {
      ...
      layoutGraph() {
          ...
          console.log('extent defined here', extent$$1);
      ...
      getSvgViewDimensions() {
          console.log('container dims', this.container.clientWidth, this.container.clientHeight);
  ...
  class ScheduleMultiView extends PhaseView {
      createViewElement() {
          const pane = document.createElement('div');
          pane.setAttribute('id', 'schedule-multiview');
          const scheduleWrapper = document.createElement('div');
          scheduleWrapper.setAttribute('id', 'text-view');
          scheduleWrapper.classList.add('wrapper', 'scrollable');
          const blockGraphWrapper = document.createElement('div');
          blockGraphWrapper.setAttribute('id', 'graph-view');
          blockGraphWrapper.classList.add('wrapper');
          const resizer = document.createElement('div');
          resizer.setAttribute('id', 'schedule-resizer');
          resizer.classList.add('resizer');
          pane.appendChild(scheduleWrapper);
          pane.appendChild(resizer);
          pane.appendChild(blockGraphWrapper);
          return pane;
      }
      constructor(parentId, broker, toolbox) {
          super(parentId);
          this.textView = new ScheduleView(this.divNode.children[0], broker);
          this.blockGraphView = new GraphView(this.divNode.children[2], broker, () => { console.log("what's this?"); }, toolbox);
      }
      initializeContent(data, rememberedSelection) {
          this.show();
          this.textView.initializeContent(data, rememberedSelection);
          this.blockGraphView.initializeContent(this.transformToGraphViewFormat(data.schedule.blocks), null);
      }
      detachSelection() {
          return this.blockGraphView.detachSelection();
      }
      onresize() {
          this.textView.onresize();
          this.blockGraphView.onresize();
      }
      searchInputAction(searchInput, e, onlyVisible) {
          this.blockGraphView.searchInputAction(searchInput, e, onlyVisible);
      }
      transformToGraphViewFormat(blocks) {
          const edges = new Array();
          const nodes = new Array();
          const getGNodeFromBlock = (blk) => {
              return new GNode(new NodeLabel(blk.id, `B${blk.id}`, 'some string', true, "", null, null, "", true, "", null));
          };
          blocks.forEach(block => {
              nodes.push(getGNodeFromBlock(block));
              block.succ.forEach(targetId => {
                  edges.push({
                      source: block.id,
                      index: 1,
                      target: targetId,
...
<div class="graph-toolbox">
  ...
  class GraphMultiView extends View {
      ...
      constructor(id, selectionBroker, sourceResolver) {
          ...
          this.schedule = new ScheduleMultiView(this.divNode, selectionBroker, toolbox.querySelector(".graph-toolbox"));
